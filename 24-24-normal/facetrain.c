...
int main()
{
    ...
    const unsigned int num_input = 24*24;
    ...
    fann_train_on_file(ann, "facetrain.txt", max_epochs,
    ...
    struct fann_train_data *data = fann_read_train_from_file("facetest.txt");
    ...
    for(i = 0; i < fann_length_train_data(data); i++) {
        ...
        for (int n=0; n<30; n++) {
            printf (" %.2f/%.2f(%.2f) ",calc_out[n]*(4*96), data->output[i][n]*(4*96), data->output[i][n]*(4*96) - calc_out[n]*(4*96)  );
